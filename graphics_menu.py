...
class MyDialog:
	def __init__(self, parent, tmsg, lmsg):
		top = self.top = tk.Toplevel(parent)
		top.title(tmsg)
		tk.Label(top, text=lmsg).pack()
#self.e = tk.Entry(top)
#self.e.pack(padx=5)
...
global dialog
global _root
def hello(tmsg = "About Info", lmsg = "Written by Chang Liu, 5110309200."):
	dialog = MyDialog(_root, tmsg, lmsg)
	_root.wait_window(dialog.top)


_root = tk.Tk()
global menubar
menubar = tk.Menu(_root)
helpmenu = tk.Menu(menubar, tearoff = 0)
helpmenu.add_command(label = "About", command = hello)
#helpmenu.add_command(label = "Quit", command = _root.destroy)
